...
class ClassementPage extends StatelessWidget {
  ...
  // Récupérer l'utilisateur connecté avec Firebase Auth
  Future<String?> getCurrentUserPseudo() async {
    User? user = FirebaseAuth.instance.currentUser;  // Récupérer l'utilisateur
    if (user != null) {
      DocumentSnapshot userDoc = await FirebaseFirestore.instance.collection('Users').doc(user.uid).get();
      return userDoc['pseudo'];  // Récupérer le pseudo de l'utilisateur
    }
    return null;
  }
              ...
              StreamBuilder<QuerySnapshot>(
                ...
                builder: (BuildContext context, AsyncSnapshot<QuerySnapshot> snapshot) {
                  ...
                  return FutureBuilder<String?>(
                    future: getCurrentUserPseudo(),  // Récupérer le pseudo de l'utilisateur connecté
                    builder: (BuildContext context, AsyncSnapshot<String?> userPseudoSnapshot) {
                      ...
                      return Column(
                        children: snapshot.data!.docs.asMap().entries.map((entry) {
                          ...
                          Color textColor = playerPseudo == currentUserPseudo ? const Color.fromARGB(255, 141, 227, 255) : Colors.white; // Si c'est l'utilisateur, couleur verte
                          ...
                          return classementCard(
                            ...
                            textColor: textColor,  // Appliquer la couleur du texte
                          );
                        }).toList(),
                      );
                    },
...
Widget classementCard({required int rank, required String name, required String url, required String points, required Color textColor}) {
  return Container(
          ...
          child: Column(
            ...
            children: [
              Text(
                name,
                style: TextStyle(
                  color: textColor,  // Couleur du texte en fonction de l'utilisateur connecté
                  fontWeight: FontWeight.bold,
                  fontSize: 18,
                ),
              ),
